...
void thread_update_pri (struct thread* t)
{
    ...
    if (!list_empty (&t->locks_keeping_list))
    {
        list_sort (&t->locks_keeping_list, lock_compare_priority, NULL);
        int lock_priority = list_entry (list_front (&t->locks_keeping_list), struct lock, list_lock)->lock_priority;
...
void thread_get_lock (struct lock *lock)
{
    ...
    cur->blocked_lock = NULL;
    lock->holder = cur;
    lock->lock_priority = cur->priority;
    list_insert_ordered(&cur->locks_keeping_list, &lock->list_lock, lock_compare_priority, NULL);
}
...
thread_set_priority (int new_priority)
{
    ...
    if (new_priority > cur->priority || list_empty (&cur->locks_keeping_list))
    {
...
init_thread (struct thread *t, const char *name, int priority)
{
  ...
  list_init (&t->locks_keeping_list);
  t->blocked_lock = NULL;
}
